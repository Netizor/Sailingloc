...
let stripe = stripeKey && /^sk_(test|live)_[a-zA-Z0-9]{20,}$/.test(stripeKey)
  ? new Stripe(stripeKey)
  : null
const PLATFORM_FEE_PERCENT = parseFloat(process.env.STRIPE_PLATFORM_FEE_PERCENT || '10')

/** Injection Stripe pour les tests d'intégration (handlers HTTP). */
export function __setStripeForTests(mock) {
  stripe = mock
}
...
/** Retourne un message d'erreur si les dates de réservation sont invalides. */
...
/**
 * Vérifie si le locataire peut réserver sans skipper quand un permis est requis.
 * CDC : vérification du permis bateau obligatoire.
 */
export function renterMeetsLicenseRequirement(boat, renter, withSkipper) {
  const needsLicense = !withSkipper && boat.required_license && boat.required_license !== 'NONE'
  if (!needsLicense) return true
  ...
  return hasQualifications || hasApprovedCv
}

/** Accès réservation : locataire, propriétaire du bateau ou admin. */
export function canAccessBooking(booking, user) {
  if (!booking || !user) return false
  const isRenter = booking.renter_id === user.id
  const isOwner = booking.boats?.owner_id === user.id
  return isRenter || isOwner || user.role === 'ADMIN'
}

/** Mappe action accept/reject → statut final. */
export function resolveBookingStatusUpdate({ status, action }) {
...
/**
 * Valide une transition de statut (acceptation / refus / clôture).
 * @returns {string|null} message d'erreur ou null
 */
export function validateBookingStatusChange({ booking, finalStatus, user }) {
  const validStatuses = ['CONFIRMED', 'CANCELLED', 'COMPLETED', 'DISPUTED']
  if (!validStatuses.includes(finalStatus)) return 'Statut invalide'

  const isOwner = booking.boats?.owner_id === user.id
  const isAdmin = user.role === 'ADMIN'

  if (finalStatus === 'CONFIRMED' && !booking.stripe_payment_intent_id && !isOwner && !isAdmin) {
    return 'Impossible de confirmer sans paiement enregistré.'
  }
  if (['CANCELLED', 'COMPLETED'].includes(booking.status) && finalStatus !== booking.status) {
    return 'Cette réservation ne peut plus être modifiée.'
  }
  return null
}

/**
 * Pré-conditions d'annulation (motif, droits, état, dates).
 * @returns {string|null}
 */
export function validateBookingCancellation({ booking, user, cancellationReason, now = new Date() }) {
  if (!cancellationReason?.trim()) return 'Le motif d\'annulation est requis'
  if (!canAccessBooking(booking, user)) return 'Accès refusé'
  if (['CANCELLED', 'COMPLETED'].includes(booking.status)) {
    return 'Cette réservation ne peut plus être annulée'
  }
  if (new Date(booking.start_date) <= now) {
    return 'Impossible d\'annuler une réservation déjà commencée'
  }
  return null
}

/**
 * Valide la confirmation de paiement Stripe avant passage en CONFIRMED.
 * @returns {string|null}
 */
export function validatePaymentConfirmation({ booking, user, bookingId, intent }) {
  if (!booking) return 'Réservation introuvable'
  if (booking.renter_id !== user.id) return 'Accès refusé'
  if (booking.status !== 'PENDING') return 'Réservation déjà traitée'
  if (!intent) return 'Paiement introuvable'
  if (intent.status !== 'succeeded') return 'Le paiement n\'a pas été confirmé par Stripe'
  if (intent.metadata?.bookingId !== String(bookingId)) {
    return 'Ce paiement ne correspond pas à la réservation'
  }
  const expectedAmount = Math.round(parseFloat(booking.total_price) * 100)
  if (intent.amount !== expectedAmount) return 'Montant du paiement incorrect'
  return null
}

/** Agrège les revenus propriétaire pour une année donnée. */
export function buildOwnerRevenuesSummary({ boats, bookings, year, now = new Date() }) {
  const yearBookings = (bookings || []).filter((b) => bookingInYear(b, year))
...
router.post('/', authenticate, async (req, res) => {
  ...
  if (!withSkipper && boat.required_license && boat.required_license !== 'NONE') {
    const { data: renter } = await supabase
      .from('users')
      .select('sailing_qualifications, sailor_cv_status, sailor_cv_doc')
      .eq('id', req.user.id)
      .single()
    if (!renterMeetsLicenseRequirement(boat, renter, withSkipper)) {
  ...
  const datesError = validateBookingDates(startDate, endDate)
...
router.get('/owner/revenues', authenticate, async (req, res) => {
  ...
  const now = new Date()
  const { summary, byMonth, byBoat } = buildOwnerRevenuesSummary({
    boats,
  ...
  const paidBookings = (allBookings || []).filter(
    (b) => bookingInYear(b, year) && (b.status === 'CONFIRMED' || b.status === 'COMPLETED'),
  )
  ...
  return res.json({ summary, byMonth, byBoat, recentBookings })
})
...
router.get('/:id', authenticate, async (req, res) => {
  ...
  if (!canAccessBooking(booking, req.user)) return res.status(403).json({ message: 'Accès refusé' })
...
router.patch('/:id/status', authenticate, async (req, res) => {
  ...
  const finalStatus = resolveBookingStatusUpdate({ status, action })

  const { data: booking } = await supabase.from('bookings').select('*, boats(owner_id)').eq('id', req.params.id).single()
  if (!booking) return res.status(404).json({ message: 'Réservation introuvable' })
  if (!canAccessBooking(booking, req.user)) return res.status(403).json({ message: 'Accès refusé' })

  const statusError = validateBookingStatusChange({ booking, finalStatus, user: req.user })
...
router.post('/:id/cancel', authenticate, async (req, res, next) => {
  try {
    ...
    const now = new Date()
    const cancelError = validateBookingCancellation({
      booking,
      user: req.user,
      cancellationReason,
      now,
    })
    if (cancelError === 'Accès refusé') return res.status(403).json({ message: cancelError })
...
router.post('/confirm-payment', authenticate, async (req, res) => {
  ...
  const paymentError = validatePaymentConfirmation({
    booking,
    user: req.user,
    bookingId,
    intent,
  })
  if (paymentError === 'Réservation introuvable') return res.status(404).json({ message: paymentError })
  if (paymentError === 'Accès refusé') return res.status(403).json({ message: paymentError })
